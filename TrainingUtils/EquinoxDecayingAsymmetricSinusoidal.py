...
class EquinoxDecayingAsymmetricSinusoidal(_LRScheduler):
    ...
    def _get_section_ceiling(self, section_index: int):
        ...
        if self.lr_decay == LrDecayMode.LINEAR:
            ...
            log_ceiling = log_lr_max - (log_lr_max - log_lr_min) * (section_index / self.n_sections)
            ceiling = math.exp(log_ceiling)
        elif self.lr_decay == LrDecayMode.COSINE:
            log_ceiling = log_lr_min + (log_lr_max - log_lr_min) * \
                        (1 + math.cos(math.pi * 0.25 * section_index / self.n_sections)) / 2
            ceiling = math.exp(log_ceiling)
        else:
    ...
    def find_peak_x(self, n, shift=0):
        return minimize_scalar(lambda x: -self.asymmetric_sinusoidal_core(n, x, shift=shift), bounds=(0, 10), method='bounded').x
    ...
    def asymmetric_sinusoidal_unit(self, n, x):
        ...
        return self.asymmetric_sinusoidal_core(n, x, wavelength, shift=first_peak_x) / amplitude_norm
