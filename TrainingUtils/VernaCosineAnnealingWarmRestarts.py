...
# from factory.IceCubeTransformer.Enum.VernaLRDecayMode import LrDecay
from Enum.VernaLRDecayMode import LrDecay


class VernaCosineAnnealingWarmRestarts(_LRScheduler):
    def __init__(self, 
                ...
                n_sections: int,
                lr_decay: LrDecay = LrDecay.LINEAR,
                last_epoch: int = -1):
    ...
    def _get_section_ceiling(self, section: int):
        height = self.lr_max - self.lr_min

        if self.lr_decay == LrDecay.LINEAR:
            ceiling = self.lr_max - height * (section / self.n_sections)
        elif self.lr_decay == LrDecay.EXPONENTIAL:
            ceiling = self.lr_max * (0.9 ** section)
        elif self.lr_decay == LrDecay.COSINE:
            ceiling = self.lr_min + (self.lr_max - self.lr_min) * (1 + math.cos(math.pi * section / self.n_sections)) / 2
        else:
            raise ValueError(f"Invalid lr_decay mode: {self.lr_decay}")  # ✅ Ensures a valid mode
